...
import { cargarGoogleMapsPublico } from '../../../services/googleMapsPublicLoader'
import { prepararPantallasPropuesta } from '../utils/prepararPantallasPropuesta'

const RADIO_BUSQUEDA_STREET_VIEW = 80

export default function StreetViewPropuestaPublica({ open, pantalla, onClose }) {
  ...
  const pantallaPreparada = useMemo(() => {
    ...
    return prepararPantallasPropuesta([pantalla]).pantallasValidas[0] || null
  }, [pantalla])
  ...
  useEffect(() => {
    ...
    async function inicializarStreetView() {
      try {
        ...
        const maps = await cargarGoogleMapsPublico()
